...
int Belt::pwm()
{
    ...
    else if (mode == 3)
    {
        ...
        {
            return static_cast<int>(DEFAULT_PWM * manualDirection * pwmPositiveDirection);
        }
...
Belt::Belt(int encoderPositiveDirection, int touchPin, long motionRange, int pwmPositiveDirection, double radius) : encoderPositiveDirection(encoderPositiveDirection), touchPin(touchPin), motionRange(motionRange), pwmPositiveDirection(pwmPositiveDirection), convertingCoefficient(2 * radius * PI / PPR) {}
...
Arm::Arm(int encoderPositiveDirection, int touchPin, long motionRange, int pwmPositiveDirection, double radius, Movable &movable) : Belt(encoderPositiveDirection, touchPin, motionRange, pwmPositiveDirection, radius), movable(movable) {}
