...
//初期セットアップを行う関数
void Cubic::begin(int8_t motornumber){ //制御したいモータを0~7の整数で指定してもらう
    ...
    if(0 <= motornumber && motornumber <8){ //正常な引数値が与えられた時
      ...
      if(motornumber == 0){ //次にdirが格納される要素を指定する
        ...
        if(motornumber < 4){
            ...
            this->encoderNum = motornumber; //encoderの番号を指定
        }
        else{
            this->encoderAddress = SLAVE_ADDRESS_ENCODER2;
            this->encoderNum = motornumber-4;
        }
    }
}

//モータを動かす関数 
bool Cubic::operator << (int data){
  ...
  //回転方向の値を代入
  if(motornum == 0){ //motor0の回転方向はパリティバイトに書き込み
    if(data < 0) g_buf[0] = 2;
    else g_buf[0] = 1;
  }
  else if(motornum == 7){ //motor7の回転方向は
    if(data < 0) g_buf[1] = (g_buf[1] & 0b11111100) | (0b11); //下位2ビットを1にする
    else g_buf[1] &= 0b11111100; //下位2ビットを0にする。
  }
  else{
    if(data < 0) g_buf[1] |= (1<<(8-motornum)); //左から(8-motornum)番目のビットを1にする
    else g_buf[1] &= ((1<<(8-motornum)) ^ 0b11111111); //左から(8-motornum)番目のビットを0にする 
  }
