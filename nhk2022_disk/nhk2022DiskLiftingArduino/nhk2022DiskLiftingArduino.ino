...
Movable movable(1, MOVABLE_TOUCH_PIN, MOVABLE_MOTION_RANGE, -1, MOVABLE_BELT_RADIUS);
Arm arm(-1, ARM_TOUCH_PIN, ARM_MOTION_RANGE, 1, ARM_BELT_RADIUS, movable);
...
void messageCb(const std_msgs::Int16MultiArray &command_msg)
{
  ...
  case 1:
    ...
    /*  setEachTargetHeight(HEIGHTS[command_msg.data[1]]); */
    break;
  case 2:
...
void loop()
{
  ...
  if (bool isInAutoMoving = movable.getMode() == 1 || arm.getMode() == 1)
  {
    ...
    {
      autoDirection = (currentArmHeight > targetHeight + ACCETABLE_ERROR) ? -1 : 0;
    }
